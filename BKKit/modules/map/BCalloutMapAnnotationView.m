...
- (id) initWithAnnotation:(id <MKAnnotation>)annotation reuseIdentifier:(NSString *)reuseIdentifier {
	if (self = [super initWithAnnotation:annotation reuseIdentifier:reuseIdentifier]) {
        ...
        _titleLabel = createLabel(CGRectZero, AnnoTitleFont, nil, AnnoTitleColor, [UIColor colorWithWhite:1.0 alpha:0.6], CGSizeMake(0, 1), NSTextAlignmentLeft, 1, NSLineBreakByTruncatingTail);
        _descLabel = createLabel(CGRectZero, AnnoSubTitleFont, nil, AnnoSubTitleColor, nil, CGSizeZero, NSTextAlignmentLeft, 1, NSLineBreakByTruncatingTail);
        ...
        UITapGestureRecognizer *tapContentView = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapContentView:)];
...
- (void)prepareFrameSize {
    ...
    float w = [anno.title sizeWithFont:AnnoTitleFont forWidth:maxWidth lineBreakMode:NSLineBreakByTruncatingTail].width;
    if (anno.subtitle) {
        w = MAX(w,[anno.subtitle sizeWithFont:AnnoTitleFont forWidth:maxWidth lineBreakMode:NSLineBreakByTruncatingTail].width);
    }
    if ([anno.smallPic isURL]) {
        w += AnnoImageWidth;
    }
    ...
    if (anno.subtitle || [anno.smallPic isURL]) {
        h += AnnoImageWidth/2;
    }
...
- (void)setAnnotation:(id <MKAnnotation>)annotation {
    BCalloutMapAnnotation *anno = (BCalloutMapAnnotation*)self.annotation;
    [self setPhotoCanFullScreen:(![[anno video] isURL] && [anno smallPic])];
...
- (void)tapContentView:(UITapGestureRecognizer *)r{
    DLOG(@"tapContentView:%ld",(long)r.state);
