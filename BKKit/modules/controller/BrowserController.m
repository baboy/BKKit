...
- (void)viewDidLoad{
	...
	rect.size.height = 14;
	self.urlLabel = createLabel(rect, [UIFont systemFontOfSize:12], nil, [UIColor colorWithWhite:0.9 alpha:1], nil, CGSizeZero, NSTextAlignmentCenter, 1, NSLineBreakByTruncatingTail);    
	...
	self.titleLabel = createLabel(rect, [UIFont boldSystemFontOfSize:14], nil, [UIColor colorWithWhite:1.0 alpha:1], nil, CGSizeZero, NSTextAlignmentCenter, 1, NSLineBreakByTruncatingTail);    
...
+ (void) open:(id)url withController:(UIViewController *)viewController{
    ...
    BrowserController *bc = [[BrowserController alloc] init];
    [bc setUrl:[url isKindOfClass:[NSURL class]]?url:[NSURL URLWithString:url]];
    XUINavigationController *nav = [[XUINavigationController alloc] initWithRootViewController:bc];
