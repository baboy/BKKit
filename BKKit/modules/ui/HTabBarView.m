...
- (UIButton *)createButtonWithFrame:(CGRect)frame info:(id)info atIndex:(NSInteger)atIndex{
...
- (void) tappedAtIndex:(NSInteger)i{
...
- (id) initWithFrame:(CGRect)frame{
	if (self = [super initWithFrame:frame]) {
		_leftLineColor = gLineTopColor ;
		_rightLineColor = gLineBottomColor;
...
- (void)setItems:(NSArray *)items{
	...
	NSInteger numOfRows = [items count];
    ...
    for (int i = 0; i < numOfRows; i++) {
        ...
        if (name && [name length] > 0) {
            ...
            UILabel *titleLabel = createLabel(titleFrame, self.titleFont, nil, self.titleColor, nil, CGSizeZero, NSTextAlignmentLeft, 1, NSLineBreakByTruncatingTail);
