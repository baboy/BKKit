...
- (UIView *)showEmptyMsg:(NSString *)msg withNetIndicator:(BOOL)withIndicator icon:(NSString *)iconName withTarget:(id)target withAction:(SEL)action{
    ...
    UILabel *msgLabel = createLabel(labelFrame, ThemeDescTextFont, nil, ThemeDescTextColor, ThemeDescTextShadowColor, CGSizeMake(0, 1), NSTextAlignmentCenter, 0, NSLineBreakByTruncatingTail);
    msgLabel.text = msg;
    if (target && action) {
