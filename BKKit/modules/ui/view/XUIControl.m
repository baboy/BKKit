...
- (void) setBackgroundImage:(UIImage *)backgroundImage forState:(UIControlState)state{
    [self.imageDict setValue:backgroundImage forKey:[NSString stringWithFormat:@"%lu",(unsigned long)UIControlStateNormal]];
    [self setNeedsDisplay];
}
- (void)drawRect:(CGRect)rect
{
    UIImage *img = [self.imageDict valueForKey:[NSString stringWithFormat:@"%lu",(unsigned long)self.state]];
    if (img) {
        [img drawInRect:rect];
    }
}
