...
- (void)setup{
    ...
    for (int i=0; i<n; i++) {
        ...
        if (![item isKindOfClass:[BDropMenuItem class]]) {
            ...
            rect.size.height = 2;
            BLineView *line = [[BLineView alloc] initWithFrame:rect];
...
- (void)show{
    ...
    if ([self.items count] && ([self.items count] > 2 || ![self.items containsObject:self.filterItem])) {
        DLOG(@"%ld",(long)[APP statusBarOrientation]);
...
- (void)setValue:(NSString *)value{
    ////
    _value = [value isKindOfClass:[NSString class]]?value:[value description];
}
- (id)get:(NSString *)key{
